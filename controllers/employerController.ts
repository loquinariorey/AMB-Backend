 ...
 */
const updateEmployerByAdmin = async (req: any, res: any, next: any) => {
  try {
    const { id } = req.params;
    const { 
      clinic_name, clinic_name_kana, business_form, 
      ...
      tel, home_page_url, access, director_name, 
      employee_number, establishment_year, business, capital_stock,
      paying_status, subscription_id, subscription_regist_date,
      subscription_release_date, customer_id, status
    } = req.body;
 ...
 */
const deleteEmployerByAdmin = async (req: any, res: any, next: any) => {
  try {
    const { id } = req.params;
    ...
    const pendingApplicationsCount = await ApplicationHistory.count({
      where: {
        job_info_id: {
          [Op.in]: Sequelize.literal(`(SELECT id FROM job_infos WHERE employer_id = ${id})`)
        }
      }
    });
 ...
 */
const restoreEmployerByAdmin = async (req: any, res: any, next: any) => {
  try {
    const { id } = req.params;
