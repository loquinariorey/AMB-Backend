...
const getAllJobs = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    let jobsWithAnalytics = allJobs.map((job: any) => job.get());
    
    try {
      const currentPageJobIds = jobsWithAnalytics.map((job: any) => job.id);
      
      // Get analytics data for current page jobs
      const currentPageAnalytics = await JobAnalytic.findAll({
        where: { job_info_id: { [Op.in]: currentPageJobIds } },
        attributes: ['job_info_id', 'search_count', 'recruits_count', 'view_count', 'favourite_count'],
        raw: true
      });
    ...
    // - favourite_count: tracked in addFavouriteJob (favorite/unfavorite actions)
      ...
      try {
        const jobIds = jobsWithAnalytics.map((job: any) => job.id);
      ...
      const analyticsMap = new Map();
      existingAnalytics.forEach((analytics: any) => {
